...
class InventoryService
{
    public static function checkAvailability(
        ...
        foreach ($detailBarang as $item) {
            if (empty($item['barang_id']) || empty($item['jumlah_barang'])) {
                continue;
            }
            ...
            if ($jumlah < 1) {
                throw new \Exception("Jumlah {$barang->nama_barang} minimal 1.");
            }

            if ($barang->status !== 'aktif') {
                throw new \Exception("Barang {$barang->nama_barang} sudah tidak aktif.");
            }
        ...
        foreach ($detailPaket as $paketItem) {
            if (empty($paketItem['paket_id']) || empty($paketItem['jumlah_paket'])) {
                continue;
            }
            ...
            if (!$paket) {
                throw new \Exception('Paket yang dipilih tidak ditemukan.');
            }

            if ($paket->status !== 'aktif') {
                throw new \Exception("Paket {$paket->nama_paket} sudah tidak aktif.");
            }

            $jumlahPaket = (int) $paketItem['jumlah_paket'];

            if ($jumlahPaket < 1) {
                throw new \Exception("Jumlah paket {$paket->nama_paket} minimal 1.");
            }

            $detail = DetailPaket::where('paket_id', $paket->id)->get();

            if ($detail->isEmpty()) {
                throw new \Exception("Paket {$paket->nama_paket} tidak memiliki barang.");
            }
            ...
            foreach ($detail as $barangPaket) {
                ...
                if (!$barang) {
                    throw new \Exception('Terdapat barang pada paket yang sudah tidak ditemukan.');
                }
                ...
                $dibutuhkan = (int) $barangPaket->jumlah * $jumlahPaket;
        ...
        // Kunci baris barang yang diminta (Pessimistic Locking)
        ...
        // 3. CEK STOK SETELAH ROW BARANG DI-LOCK
        foreach ($kebutuhanBarang as $barangId => $jumlahDibutuhkan) {
            ...
            $stokTersedia = (int) $barang->jumlah_total - (int) $dipakai;

            if ($jumlahDibutuhkan > $stokTersedia) {
                throw new \Exception(
                    "Stok {$barang->nama_barang} tidak mencukupi. " .
                        "Stok tersedia {$stokTersedia}, sedangkan kebutuhan {$jumlahDibutuhkan}."
                );
            }
        }
    }

    // Menghitung jumlah barang yang sedang digunakan dengan Current / Locking Read
    private static function jumlahBarangDipakai(
        ...
        // Menggunakan lockForUpdate() agar query ini menjadi CURRENT READ (bukan snapshot lama)
        $penyewaan = $query->lockForUpdate()->get();
            ...
            foreach ($item->detailPaket as $paket) {
                ...
                foreach ($paket->paket->detail as $isi) {
                    if ($isi->barang_id == $barangId) {
                        $total += (int) $isi->jumlah * (int) $paket->jumlah_paket;
                    }
                }
            }
        }
    ...
    public static function getAvailableToday($barangId)
    {
        ...
        $dipakai = self::jumlahBarangDipakai($barangId, $hariIni, $hariIni);
        $tersedia = $barang->jumlah_total - $dipakai;

        return max(0, $tersedia);
    }

    public static function getAvailableStock($barangId, $mulai, $selesai)
    {
        $barang = Barang::find($barangId);
        ...
        $dipakai = self::jumlahBarangDipakai($barangId, $mulai, $selesai);
        $tersedia = $barang->jumlah_total - $dipakai;
